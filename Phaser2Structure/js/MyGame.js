window.MyGame = window.MyGame || (function(myGameWindowObject) {'use strict';
    var windowObject = myGameWindowObject,
        thisObject = null,
        score = 0,
        currentLevel = 1,
        gameProgress = 1,
	...
	addConstant: function (MyObject, constantName, constantValue) {
        ...
        namespace: function (namespaceStr, inheritObject, newObject) {
            ...
            if (parts[0] === myGameObject.Const.GAME_NAME) {
                parts = parts.slice(1);
            }

            for (i = 0; i < parts.length; i += 1) {
                if (myGameObject[parts[i]] === undefined) {
                    for (prop in newObject) {
                        if (newObject.hasOwnProperty(prop)) {
                            helpObject[prop] = newObject[prop];
                        }
                    }
                    myGameObject[parts[i]] = helpObject;
                }
                myGameObject = myGameObject[parts[i]];
            }
            return myGameObject;
        },
...
window.document.addEventListener('deviceready', function (event) {'use strict';
