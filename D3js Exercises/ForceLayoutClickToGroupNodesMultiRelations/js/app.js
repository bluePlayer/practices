 ...
 * @param {Object} d3
 */
window.ForceLayoutClickToGroupNodesMultiRelations = window.ForceLayoutClickToGroupNodesMultiRelations || (function(d3) {
    'use strict';
    ...
    return {
        ...
        network: function(data, prev) {
            ...
            for (k in lm) {
                if (lm.hasOwnProperty(k)) {
                    links.push(lm[k]);
                }
            }

            for (k in lml) {
                if (lml.hasOwnProperty(k)) {
                    helper_links.push(lml[k]);
                }
            }

            for (k in lmm) {
                if (lmm.hasOwnProperty(k)) {
            ...
            for (k in lmr) {
                if (lmr.hasOwnProperty(k)) {
                    helper_links.push(lmr[k]);
                }
            }

            for (k in nml) {
                if (nml.hasOwnProperty(k)) {
                    helper_nodes.push(nml[k]);
                }
            }

            for (k in nmr) {
                if (nmr.hasOwnProperty(k)) {
